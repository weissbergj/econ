...
def load_and_prepare_data():
    ...
    state_industry['wage_inequality'] = state_industry['wage_gini']
...
def create_summary_statistics(df):
    ...
    # 2. Key Variables Statistics - removed wage_inequality_ratio
    key_vars = ['workers', 'median_wage', 'mean_wage', 'wage_inequality']
    ...
    # Format numbers - no need to check for ratio anymore
    for col in var_stats.columns:
        var_stats[col] = var_stats[col].apply(lambda x: format_number(x))
...
def run_section_five(df):
    ...
    for measure in ['wage_ratio', 'top_bottom_ratio', 'mid_small_ratio']:
        ...
        valid_data = df.dropna(subset=[measure, 'workers', 'digital', 
                                     'post_2015', 'digital_x_post2015'])
    ...
    size_q1 = df['workers'].quantile(0.25)
    size_q3 = df['workers'].quantile(0.75)
    size_iqr = size_q3 - size_q1
    size_upper = size_q3 + (1.5 * size_iqr)
    size_lower = size_q1 - (1.5 * size_iqr)
    df_no_outliers = df_no_outliers[
        (df_no_outliers['workers'] >= size_lower) & 
        (df_no_outliers['workers'] <= size_upper)
    ]
...
#     # # 8. Digital Transformation Analysis THIS IS THE MAIN DDD!!!
#     # print("\n=== Digital Transformation Analysis ===")
#     # dig_results, trend_results = analyze_digital_transformation(state_industry)
#     # print("\nTriple-Difference Results:")
#     # print(dig_results.summary.tables[1])
...
#     # Heterogeneity/ML Models
#     analyze_heterogeneity(state_industry)
