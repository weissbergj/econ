...
def verify_data_quality(df):
    ...
    assert df['YEAR'].max() <= 2021, "Data includes years after 2021"
    ...
    # 3. Key Variables Summary (UPDATED)
    print("\nKey Variables Summary:")
    for var in ['establishments_01: Total', 'establishments_09: 500+', 'median_wage', 'mean_wage', 'total_wage_bill']:
        print(f"\n{var}:")
    ...
    # 5. Time Series Completeness (UPDATED)
    ...
    # 6. Check for Suspicious Values (UPDATED)
    print("\nSuspicious Values Check:")
    print("Negative wages:", (df['median_wage'] <= 0).sum())
    print("Zero total establishments:", (df['establishments_01: Total'] <= 0).sum())
    print("Mean < Median wage:", (df['mean_wage'] < df['median_wage']).sum())
    
    # 7. Check for Outliers (UPDATED)
    print("\nPotential Outliers (large establishments):")
    df['large_estab_share'] = df['establishments_09: 500+'] / df['establishments_01: Total']
    q1 = df['large_estab_share'].quantile(0.25)
    q3 = df['large_estab_share'].quantile(0.75)
    iqr = q3 - q1
    outliers = df[(df['large_estab_share'] < (q1 - 1.5 * iqr)) | (df['large_estab_share'] > (q3 + 1.5 * iqr))]
...
def clean_data(df):
    ...
    df['large_estab_intensity'] = weighted_sum
...
def load_and_prepare_data():
    ...
    # Remove 2022 data FIRST, before any other processing
    state_industry = state_industry[state_industry['YEAR'] <= 2021]
    ...
    # Clean data
    state_industry = clean_data(state_industry)  # Note: clean_data also enforces <= 2021
...
def run_all_regressions(state_industry, industry_dummies):
    ...
    model = PanelOLS(
        ...
        exog=sm.add_constant(panel_data[['large_estab_intensity']]),
        entity_effects=True,
        time_effects=True,
        check_rank=False
    )
    ...
    # Recreate industry dummies from cleaned data to ensure alignment
    industry_dummies = pd.get_dummies(state_industry['industry_name'], prefix='ind', drop_first=True)
    exog_data = pd.concat([industry_dummies, state_industry[['large_estab_intensity']]], axis=1)
    ...
    for ind in major_industries:
        state_industry[f'size_x_{ind}'] = (
            state_industry['large_estab_intensity'] * 
            (state_industry['industry_name'] == ind)
        )
    
    interaction_data = sm.add_constant(
        state_industry[[f'size_x_{ind}' for ind in major_industries] + ['large_estab_intensity']]
    )
...
def analyze_time_trends(df):
    ...
    df['size_x_post2008'] = df['large_estab_intensity'] * df['post_2008']
    
    # Pre/Post COVID
    df['post_covid'] = df['YEAR'] >= 2020
    df['size_x_postcovid'] = df['large_estab_intensity'] * df['post_covid']
    
    # Time trend
    df['time_trend'] = df['YEAR'] - df['YEAR'].min()
    df['size_x_trend'] = df['large_estab_intensity'] * df['time_trend']
...
def analyze_geographic_variation(df):
    ...
    for region in regions.keys():
        df[f'size_x_{region}'] = df['large_estab_intensity'] * (df['region'] == region)
...
def run_robustness(df):
    ...
    df['large_estab_intensity_squared'] = df['large_estab_intensity'] ** 2
    df['large_estab_intensity_cubed'] = df['large_estab_intensity'] ** 3
    ...
    # Size categories
    df['size_category'] = pd.qcut(df['large_estab_intensity'], q=5, labels=['Very Small', 'Small', 'Medium', 'Large', 'Very Large'])
...
def test_regional_differences(results):
    ...
    coef_data = {
        region: {
            'coef': results['regional'][region].params['large_estab_intensity'],
            'se': results['regional'][region].std_errors['large_estab_intensity']
        }
...
def run_extended_regressions(df):
    ...
    periods_data['size_post2015'] = periods_data['large_estab_intensity'] * periods_data['post_2015']
    
    model = PanelOLS(
        dependent=periods_data['wage_inequality'],
        exog=sm.add_constant(periods_data[['large_estab_intensity', 'size_post2015']]),
        entity_effects=True,
        time_effects=False,
        check_rank=False
    )
    ...
    nonlinear_data['large_estab_intensity_squared'] = nonlinear_data['large_estab_intensity'] ** 2
    
    model = PanelOLS(
        dependent=nonlinear_data['wage_inequality'],
        exog=sm.add_constant(nonlinear_data[['large_estab_intensity', 'large_estab_intensity_squared']]),
        entity_effects=True,
        time_effects=True,
        check_rank=False
    )
        ...
        if len(region_data) > 0:
            try:
                model = PanelOLS(
                    ...
                    exog=sm.add_constant(region_data[['large_estab_intensity']]),
                    entity_effects=True,
                    time_effects=True,
                    check_rank=False
                )
...
def run_time_effects(panel_data):
    ...
    panel_data['size_x_covid'] = panel_data['large_estab_intensity'] * panel_data['post_covid']
    
    # Run regression with COVID interaction
    model = PanelOLS(
        dependent=panel_data['wage_inequality'],
        exog=sm.add_constant(panel_data[['large_estab_intensity', 'size_x_covid']]),
        entity_effects=True,
        time_effects=True,
        check_rank=False
    )
    ...
    covid_total_effect = results.params['large_estab_intensity'] + results.params['size_x_covid']
    covid_se = (results.std_errors['large_estab_intensity']**2 + results.std_errors['size_x_covid']**2)**0.5
...
def create_summary_statistics(df):
    ...
    key_vars = ['large_estab_intensity', 'median_wage', 'mean_wage', 'wage_inequality', 'wage_inequality_ratio']
    ...
    quality_stats = pd.DataFrame({
        ...
        'Count': [
            ...
            len(identify_outliers(df['large_estab_intensity'])),
    ...
    regional_stats = df.groupby('region').agg({
        ...
        'large_estab_intensity': ['mean', 'median']
    })
    regional_stats.columns = ['Observations', 'Share', 'States', 'Industries', 'Mean Size', 'Median Size']
    regional_stats = regional_stats.map(lambda x: format_number(x) if isinstance(x, (int, float)) and not isinstance(x, str) else x)
    ...
    for period, mask in time_periods.items():
        ...
        time_stats.append({
            ...
            'Mean Workers': format_number(period_data['large_estab_intensity'].mean()),
...
def create_main_visualization(df):
    ...
    df = df.copy()
    df['log_large_estab_intensity'] = np.log(df['large_estab_intensity'])
    df['size_category'] = pd.qcut(df['log_large_estab_intensity'], q=3, 
...
def run_nonlinear_analysis(df):
    """Run non-linear analysis of establishment size effects"""
    ...
    # 1. Quadratic and Cubic Specifications
    print("\n1. Quadratic and Cubic Specification Results:")
    panel_data['large_estab_intensity_squared'] = panel_data['large_estab_intensity'] ** 2
    panel_data['large_estab_intensity_cubed'] = panel_data['large_estab_intensity'] ** 3
    
    # Quadratic model
    model = PanelOLS(
        dependent=panel_data['wage_inequality'],
        exog=sm.add_constant(panel_data[[
            'large_estab_intensity',
            'large_estab_intensity_squared'
        ]]),
        entity_effects=True,
        time_effects=True
    )
    results['quadratic'] = model.fit(cov_type='clustered', cluster_entity=True)
    ...
    # 2. Log Specification (with safety checks)
    print("\n2. Log Specification Results:")
    # Add small constant to avoid log(0)
    min_non_zero = panel_data['large_estab_intensity'][panel_data['large_estab_intensity'] > 0].min()
    panel_data['log_estab_intensity'] = np.log(panel_data['large_estab_intensity'] + min_non_zero/2)
    
    try:
        model = PanelOLS(
            dependent=panel_data['wage_inequality'],
            exog=sm.add_constant(panel_data['log_estab_intensity']),
            entity_effects=True,
            time_effects=True
        )
        ...
        print(f"Warning: Log specification failed with error: {str(e)}")
    
    # 3. Spline/Threshold Effects
    print("\n3. Threshold Effects:")
    quartiles = panel_data['large_estab_intensity'].quantile([0.25, 0.5, 0.75])
    panel_data['high_intensity'] = (panel_data['large_estab_intensity'] > quartiles[0.75]).astype(float)
    panel_data['low_intensity'] = (panel_data['large_estab_intensity'] < quartiles[0.25]).astype(float)
    
    model = PanelOLS(
        dependent=panel_data['wage_inequality'],
        exog=sm.add_constant(panel_data[['high_intensity', 'low_intensity']]),
        entity_effects=True,
        time_effects=True
    )
    results['threshold'] = model.fit(cov_type='clustered', cluster_entity=True)
    print(results['threshold'].summary.tables[1])
...
def print_nonlinear_results(results):
    ...
    if 'quadratic' in results:
        print("\n1. Quadratic Model Results:")
        print(results['quadratic'].summary.tables[1])
    
    if 'cubic' in results:
        print("\n2. Cubic Model Results:")
...
def run_diff_in_diff(df):
    ...
    for industry in df['industry_name'].unique():
        ...
        df[f'size_x_ind_post2015'] = df['large_estab_intensity'] * df[f'ind_post2015']
        df[f'size_x_ind_covid'] = df['large_estab_intensity'] * df[f'ind_covid']
        
        # Create year interactions for parallel trends
        for year in range(2010, 2015):
            df[f'year_{year}'] = (df['YEAR'] == year).astype(int)
            df[f'size_x_ind_{year}'] = df['large_estab_intensity'] * df[f'{industry}_ind'] * df[f'year_{year}']
        ...
        model_trends = PanelOLS(
            dependent=panel_data['wage_inequality'],
            exog=sm.add_constant(panel_data[['large_estab_intensity', f'{industry}_ind'] + pre_trend_vars]),
            entity_effects=True,
            time_effects=True,
            check_rank=False,
            drop_absorbed=True
...
def run_event_study(df):
    ...
    for year in years:
        if year != base_year:
            ...
            for industry in key_industries:
                ...
                new_cols[f'size_x_{industry}_x_{year}'] = new_cols[f'{industry}_x_{year}'] * df['large_estab_intensity']
    ...
    for industry in key_industries:
        ...
        model = PanelOLS(
            dependent=panel_data['wage_inequality'],
            exog=sm.add_constant(panel_data[['large_estab_intensity'] + year_interactions]),
            entity_effects=True,
            time_effects=True
        )
...
def plot_event_study(result, industry, base_year):
    ...
    coef = result.params[2:]  # Skip constant and large_estab_intensity
...
def run_placebo_tests(df):
    ...
    for year in placebo_years:
        ...
        df[f'size_x_post_{year}'] = df['large_estab_intensity'] * df[f'post_{year}']
...
def analyze_industry_characteristics(df):
    ...
    df['large_estab_intensity_x_digital_intensity'] = df['large_estab_intensity'] * df['digital_intensity']
    df['post_2015'] = (df['YEAR'] >= 2015).astype(int)
    ...
    model = PanelOLS(
        ...
        exog=sm.add_constant(panel_data[[
            'large_estab_intensity',
            'large_estab_intensity_x_digital_intensity',
...
def analyze_mechanisms(df):
    ...
    df['organizational_complexity'] = (df['large_estab_intensity'] / df.groupby('YEAR')['large_estab_intensity'].transform('mean')).fillna(0)
    
    # Create interactions
    mechanisms = ['automation_exposure', 'skill_intensity', 'organizational_complexity']
    for mechanism in mechanisms:
        df[f'large_estab_intensity_x_{mechanism}'] = df['large_estab_intensity'] * df[mechanism]
    ...
    for mechanism in mechanisms:
        model = PanelOLS(
            ...
            exog=sm.add_constant(panel_data[[
                'large_estab_intensity',
                f'large_estab_intensity_x_{mechanism}'
            ]]),
            entity_effects=True,
            time_effects=True,
            drop_absorbed=True
...
def analyze_digital_transformation_robust(df):
    ...
    df['period'] = pd.cut(df['YEAR'], 
                         bins=[2007, 2009, 2011, 2013, 2015, 2017, 2019, 2021],
...
def analyze_digital_transformation(df):
    ...
    df['size_x_digital_x_post2015'] = df['large_estab_intensity'] * df['digital'] * df['post_2015']
    ...
    for year in range(-5, 0):  # 2010-2014
        ...
        pre_2015[f'size_x_digital_x_t{year}'] = pre_2015['large_estab_intensity'] * pre_2015[f'digital_x_t{year}']
    ...
    trend_model = PanelOLS(
        ...
        exog=sm.add_constant(panel_pre[['large_estab_intensity'] + trend_vars]),
        entity_effects=True,
        time_effects=True,
        drop_absorbed=True
    )
...
def plot_digital_transformation_effect(df, results):
    ...
    plt.text(2016, plt.gca().get_ylim()[1]*0.9,
             'Post-2015:\n+$1,732 level shift\n+$0.38 per large_estab_intensity',
...
def plot_event_study_results(df):
    ...
    periods = ['2007', '2008-09', '2010-11', '2012-13', '2015', '2016-17', '2018-19', '2020-21']
    
    # Create period bins including 2007
    df['period'] = pd.cut(df['YEAR'], 
                         bins=[2006, 2007, 2009, 2011, 2013, 2015, 2017, 2019, 2021],
                         labels=periods,
                         include_lowest=True)
...
def print_regression_results(results):
    ...
    print(f"large_estab_intensity coefficient: {results.params['large_estab_intensity']:.4f} (p={results.pvalues['large_estab_intensity']:.4f})")
...
def verify_event_study_data(results, state_industry):
    ...
    print("\n1. Base Effects:")
    print(f"large_estab_intensity effect: {results.params['large_estab_intensity']:.4f}")
    ...
    # 3. Sample statistics
    print("\n3. Sample Statistics:")
    print("\nMean large_estab_intensity by group:")
    print(state_industry.groupby('digital')['large_estab_intensity'].mean())
    
    # 4. Calculated effects
    print("\n4. Total Effects at Mean Size:")
    mean_large_estab_intensity = state_industry['large_estab_intensity'].mean()
    print(f"Mean establishment size: {mean_large_estab_intensity:.1f}")
    
    for t in [-2, -1, 0, 1, 2, 3]:
        base = results.params[f'digital_x_t{t}']
        size_int = results.params[f'size_x_digital_x_t{t}']
        total = base + (size_int * mean_large_estab_intensity)
...
def calculate_actual_inequality(df):
    ...
    df['period'] = pd.cut(df['YEAR'], 
                         bins=[2007, 2009, 2011, 2013, 2015, 2017, 2019, 2021],
                         labels=['2008-09', '2010-11', '2012-13', '2015', '2016-17', '2018-19', '2020-21'])
...
def calculate_controlled_differences(results, df):
    ...
    mean_large_estab_intensity = df['large_estab_intensity'].mean()
    print(f"Mean large_estab_intensity: {mean_large_estab_intensity}")  # Add this debug print
    ...
    for year in years:
        ...
        if year <= 2009:
            ...
            effect = base + (size_int * mean_large_estab_intensity)
        elif year <= 2013:
            period = 't-1'
...
def plot_controlled_event_study_results(df, results):
    ...
    for i, period in enumerate(periods):
        if period == '2007' or period == '2008-09':
            ...
            base = results.params['digital_x_t-2']
            size_int = results.params['size_x_digital_x_t-2']
            effect = base + (size_int * mean_large_estab_intensity)  # Should be -299.91
        elif period == '2012-13':
            base = results.params['digital_x_t-1']
            size_int = results.params['size_x_digital_x_t-1']
            effect = base + (size_int * mean_large_estab_intensity)  # Should be 74.16
        elif period == '2015':
            base = results.params['digital_x_t0']
            size_int = results.params['size_x_digital_x_t0']
            effect = base + (size_int * mean_large_estab_intensity)  # Should be 670.12
        elif period == '2016-17':
            base = results.params['digital_x_t1']
            size_int = results.params['size_x_digital_x_t1']
            effect = base + (size_int * mean_large_estab_intensity)  # Should be 1434.72
        elif period == '2018-19':
            base = results.params['digital_x_t2']
            size_int = results.params['size_x_digital_x_t2']
            effect = base + (size_int * mean_large_estab_intensity)  # Should be 1858.04
        else:  # 2020-22
            base = results.params['digital_x_t3']
            size_int = results.params['size_x_digital_x_t3']
            effect = base + (size_int * mean_large_estab_intensity)  # Should be 3043.96
...
def load_and_prepare_data_establishment():
    ...
    # df['large_estab_share'] = df['establishments_09: 500+'] / df['establishments_01: Total'] // FIX IN FUTURE
...
def main():
    ...
    state_industry['wage_inequality_ratio'] = state_industry['wage_inequality_ratio'].replace([np.inf, -np.inf], np.nan)
    state_industry = state_industry.dropna(subset=['wage_inequality_ratio', 'wage_inequality', 'large_estab_intensity'])
